...
> 这里需要注意的是，采用 MappedByteBuffer 这种内存映射的方式有几个限制，其中之一是一次只能映射1.5~2G 的文件至用户态的虚拟内存，这也是为何RocketMQ默认设置单个CommitLog日志数据文件为1G的原因了
...
* IndexFile：为了消息查询提供了一种通过 key 或时间区间来查询消息的方法，这种通过 IndexFile 来查找消息的方法不影响发送与消费消息的主流程（也是存放索引文件的目录，但是索引方式和CosumerQueue不同。ConsumerQueue更常用）
...
上述负载均衡的前提是这些消息都属于同一个主题（Topic）

### 1.3.2 Consumer负载均衡

#### 1）集群模式

在集群消费模式下，每条消息只需要投递到订阅这个topic的Consumer Group下的一个实例即可。RocketMQ采用主动拉取的方式拉取并消费消息，在拉取的时候需要明确指定拉取哪一条 message queue。
...
但是如果 consumer 实例的数量比 message queue 的总数量还多的话，多出来的 consumer 实例将无法分到queue，也就无法消费到消息，也就无法起到分摊负载的作用了。所以**需要控制让queue的总数量大于等于consumer的数量**。
...
```java
// 此配置方式是rocketmq的，不是spring-rocketmq的
...
**Action已经没了，现在用的是ConsumeConcurrentlyStatus。而且MessageListener接口也没有定义任何方法。用的是MessageListenerOrderly和MessageListenerConcurrently**
...
**解析配置文件**，填充NameServerConfig、NettyServerConfig属性值，并**创建NamesrvController对象**
...
//解析启动-c参数  （在命令行中启动namesrv -c选项可以重新指定配置文件的路径）
...
//解析启动-p参数 （在命令行中启动namesrv -p选项可以指定配置文件中某个属性的值）
...
根据启动属性创建NamesrvController实例，**初始化该实例**。NameServerController实例为NameServer核心控制器
...
public boolean initialize() {
    ...
    // 开启定时任务：每个10秒扫描一次保存Broker的心跳的表，然后干掉120s内都没有发送心跳的Broker
...
NameServer维护了一些集合，这些集合包含Topic的路由信息，Broker的状态信息等
...
这里要讲的是，此类会解析请求类型，如果请求类型是为***REGISTER_BROKER***，则将请求转发到`RouteInfoManager#regiesterBroker`
...
case RequestCode.REGISTER_BROKER:
	...
	// 这里有一个版本的控制。对于来自版本大于等于V3_0_11的Broker的心跳执行这里的方法
	if (brokerVersion.ordinal() >= MQVersion.Version.V3_0_11.ordinal()) {
	    return this.registerBrokerWithFilterServer(ctx, request);
	} else {
        // 来自其他版本的Broker的心跳请求执行这个方法
        // 注册Broker信息
...
校验消息的目的：对消息，主题名判空，校验消息指定的主题是否是默认主题，判断消息大小是否超过限制
...
查找路由的目的：根据消息的主题，为消息查找一个合适的Broker，并将消息发给选定的Broker
...
选择队列的目的：查找路由返回的Broker中，有很多MessageQueue，需要从这些队列中选出一个队列作为消息的目的地
...
消息存储的核心类是store模块下的org.apache.rocketmq.store.DefaultMessageStore
...
// 刷盘。在刷盘前，消息都是被保存到内存中，刷盘才是真正将消息写入磁盘的操作
...
- index：消息索引文件存储目录
- abort：启动Broker时会创建此文件，Broker正常关闭时uhi删除该文件。如果时非正常关闭，则不会删除此文件
...
int mappedFileSize;	// 单个文件大小，默认1G
CopyOnWriteArrayList<MappedFile> mappedFiles;	// MappedFile文件集合。对应commitLog文件夹里的所有文件
...
MappedFileQueue提供了一些查找指定MappdeFile对象的方法，和获取最大最小偏移量的方法
...
- RocketMQ中的路由指的是Topic路由，但是Topic路由是什么？
- 在2.2.3.4讲的路由发现中，路由变化后，NameServer不会主动通知客户端，而是由客户端主动向NameServer发送请求获取路由。这个过程叫做路由发现。问题是为什么客户端需要路由发送的功能？Producer和Consumer 生产消息，获取消息。至于有没有对应的Topic是由NameServer保存，具体的Topic由Broker持有，为什么客户端需要知道路由？为什么需要由路由发现的功能？
