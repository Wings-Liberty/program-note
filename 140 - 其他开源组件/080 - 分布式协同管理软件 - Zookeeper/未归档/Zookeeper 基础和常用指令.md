...
- 结点的数据：即`znode data `（结点`path`和结点`data`的关系就像是`Java map `中的 `key value `关系）
- 结点的子结点`children`
...
`create [-s] [-e] path data` # 其中 -s 为有序结点，-e 临时结点（默认是持久结点）  data带不带双引号的效果是一样的，都被当作字符串处理

创建 -s 有序节点，就是在创建节点时，帮你在节点名后追加一个自增需要。例如创建` creat -s /a a `结果创建的是`/a00000002`
...
```shell
delete /hadoop           # 删除结点
delete /hadoop 1         # 乐观锁机制，与 set 方法一致。当传入的版本号和最新的版本不一致时，zookeeper会拒绝本词删除
```
...
PS：哪个客户端创建的监听器，发生事件时就向哪个客户端发送事件。A创建的监听器，发生事件时，zk 只向 A 发送事件，不向 B 发送事件

```shell
get /hadoop watch        # get 的时候添加监听器，当值改变的时候，监听器返回消息
get -w /hadoop           # get path [watch] 已被弃用，现使用 -w 选项
set /hadoop 45678        # 测试
```
...
- ```shell
  ...
  addauth digest itcast:123456      # 认证，密码错了， zookeeper 不会报错，但是不能认证（添加认证用户和认证均使用addauth指令）
  get /hadoop
  ```
...
zookeeper 的 Java 客户端有三种

- zookeeper 提供的（比较底层，使用起来比较繁琐）
- 阿帕奇提供的 Apache Curator   推荐使用
- 开源的 zkclient  生产环境下不推荐使用
...
这个是某个开源的zkclient，放这里就是为了备忘。下面使用的 API 是 zk官方提供的客户端（也就是上面那个 maven 依赖）
...
- `connectionString` - `zookeeper `的地址   host:port
...
```shell
tickTime=2000  # 发送心跳的时间单位。时间单位为毫秒
dataDir=/var/lib/zookeeper
clientPort=2181
initLimit=5  # 集群中Follower和Leader 第一次连接时的超时时间，如果第一次连接时超过5个tickTime还没收到心跳响应表示主从连接失败
syncLimit=2  # 集群中Leader与Follower之间的最大响应时间单位。如果超过syncLimit * tickTime后Leader还没有收到Follower的犀鸟响应，表示Follower挂掉，将其从列表中删除
...
> Apache Curator 的几个组成部分
>
> - Client：是 ZooKeeper客户端的个替代品,提供了一些底层处理和相关的工具方法
> - Framework：用来简化 ZooKeeper高级功能的使用,并增一些新的功能,比如管理到 ZooKeeper集群的连接,重试处理
> - Recipes:实现了通用 ZooKeeper的 grecipe,该组件建立在 Framework的基础之上
> - Utilties：各种 ZooKeeper的工具类
> - Errors：异常处理，连接，恢复等
> - Extensions：recipe扩展
