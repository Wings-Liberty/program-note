...
mybatis 3.4 后完善了大量的日期类型转换器，所以无需再手动写 JDK1.8 LocalTime，LocalDate，LocalDateTime 的类型转换器
...
分段查询中`Emp`类中的成员变量`Dpt`的值由分段查询中的第二条sql查询出来。开启懒加载后，分段查询只会执行第一条sql。当需要成员变量`Dpt`中的数据时才会执行（自动）第二条sql（即使用到它时次啊会执行sql，提高了性能）
